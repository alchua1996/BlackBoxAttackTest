...
args["lr_idx"] = 0
args["lr"] =  0.01  #0.02 for uncons
args["constraint"] = 'cons' #'uncons'
args["mode"] = "ZOAdaBound" 
...
def main(args):
    with tf.Session() as sess:
        ...
        while iii < args["image_number"]:
            ...
            if arg_mode == "ZOAdaMM" or arg_mode == "ZOAdaBound":
            ...
            for i in range(I):
                ...
                if arg_mode == "ZOAdaBound":
                    nu_min = 1e-3 - 1/((i+1)**(1.5) + 1)
                    nu_max = 1e-3 + 1/(i+1)**(1.5)
                    m = beta_1 * m + (1-beta_1) * grad_est
                    v = beta_2 * v + (1 - beta_2) * np.square(grad_est) ### vt
                    v_hat = np.maximum(v_hat,v)
                    nu = np.clip(base_lr * np.ones_like(v)/np.sqrt(v), nu_min, nu_max)
                    #print(nu)
                    #print(base_lr * np.ones_like(v)/np.sqrt(v))
                    delta_adv = delta_adv - m * nu
                    if args["constraint"] == 'cons':
                        tmp = delta_adv.copy()
                        delta_adv = projection_box(tmp, orig_img_vec, 1/(nu.reshape(1,-1)), -0.5, 0.5)
                if arg_mode == "ZOSMD":

...
class NodeLookup(object):
  ...
  def load(self, label_lookup_path):
    ...
    for line in proto_as_ascii:
      if line:
        name = line.replace('\n' ,"")
        node_id_to_name[count] = name
        count+=1
    
    v = []    
    for value in node_id_to_name.items():
        v.append(value)
...
def create_graph():
  ...
  with tf.gfile.FastGFile(os.path.join(
    #  FLAGS.model_dir, 'classify_image_graph_def.pb'), 'rb') as f:
      FLAGS.model_dir, 'inception_v3_2016_08_28_frozen.pb'), 'rb') as f:
...
def readimg(ff):
  f = "/mnt/ufs18/home-253/chuaalbe/ZO-AdaBound/imagesnetdata/imgs/"+ff
...
class ImageNet:
  def __init__(self, seed):
    ...
    pool = Pool(8)
    file_list = sorted(os.listdir("/mnt/ufs18/home-253/chuaalbe/ZO-AdaBound/imagesnetdata/imgs/"))
    random.seed(seed)
...
class ImageNet_Universal:
  def __init__(self, seed):
    ...
    #random.seed(seed)
    #random.shuffle(file_list)
    r = pool.map(readimg, file_list[:1000])
    #print(file_list[:200])
    #random.shuffle(file_list)
    r = [x for x in r if x != None]
    test_data, test_labels = zip(*r)

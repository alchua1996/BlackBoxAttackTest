...
def main(args):
    with tf.Session() as sess:
        ...
        for i in range(I):
            ...
            if arg_mode == "ZOAdaBound":
                nu_min = 0.1 - 0.1/((1-beta_2)*(i+1) + 1)
                nu_max = 0.1 + 0.1/((1-beta_2)*(i+1) + 1)
                m = beta_1 * m + (1-beta_1) * grad_est
                v = beta_2 * v + (1 - beta_2) * np.square(grad_est) ### vt
                nu = np.clip(base_lr * np.ones_like(v)/np.sqrt(v), nu_min, nu_max)/np.sqrt(i+1)
                delta_adv = delta_adv - base_lr * m * nu
                if args["constraint"] == 'cons':
                    tmp = delta_adv.copy()
                    nu_temp = 1.0/nu.reshape(1,-1)
                    delta_adv = projection_box(tmp, orig_img_vec, nu_temp, -0.5, 0.5)
            if arg_mode == "ZOSMD":
            ...
            if args["print_iteration"]:
                if np.remainder(i + 1, 100) == 0:
                    if (true_label != np.argmax(attack_prob,1)).all():
                        ...
                        sr = np.sum(np.where(true_label != np.argmax(attack_prob,1), 1, 0))/arg_bsz
...
def projection_box_2(a_point, X, Vt, lb, up):
    ## X \in R^{d \times m}
    d_temp = a_point.size
    VtX = np.matmul(np.sqrt(Vt), X)

    min_VtX = np.min(VtX, axis=1)
    max_VtX = np.max(VtX, axis=1)

    Lb = lb * np.matmul(np.sqrt(Vt),np.ones((d_temp,1)) - min_VtX.reshape((-1,1)))
    Ub = up * np.matmul(np.sqrt(Vt),np.ones((d_temp,1)) - max_VtX.reshape((-1,1)))
    
    a_temp = np.matmul(np.sqrt(Vt),(a_point.reshape((-1,1))))
    
    z_proj_temp = np.multiply(Lb, np.less(a_temp, Lb)) + np.multiply(Ub, np.greater(a_temp, Ub)) \
                  + np.multiply(a_temp, np.multiply( np.greater_equal(a_temp, Lb), np.less_equal(a_temp, Ub)))
    
    delta_proj = np.matmul(np.diag(1/np.diag(np.sqrt(Vt))), z_proj_temp)
    #print(delta_proj)
    return delta_proj.reshape(a_point.shape)
